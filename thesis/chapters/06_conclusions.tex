...
\todos{Reescribir conclusiones con el nuevo orden: ANN baseline/Sobolev como bloque principal; CaNN frente a Liu; PINN baseline/Sobolev/Sobolev+ACV como bloque physics-informed complementario.}
...
\todos{Actualizar contribuciones para que ANN-IV Sobolev y comparacion con estado del arte aparezcan explicitamente como aportacion central.}
...
\todos{Revisar contribucion PINN: presentarla como analisis complementario de baseline/Sobolev/Sobolev+ACV, no por delante del resultado ANN Sobolev.}
...
\todos{Anadir limitacion: Sobolev y ACV introducen informacion derivativa/analitica adicional, por lo que no deben venderse igual que la PINN baseline no supervisada.}
...
A third limitation is the payoff kink itself. The ACV result reduces the short-maturity ATM Greek error substantially, but it does not remove the mathematical difficulty created by the non-smooth terminal payoff. In particular, the extremely short-maturity bucket \(\tau<5\times10^{-4}\) still requires careful validation of the reference Gamma before making stronger claims about that layer.
...
\todos{Actualizar future work con la nueva prioridad: validar ANN Sobolev en mercado real, evaluar hedging con griegas ANN/PINN, y explorar PINN mas puramente no supervisada para griegas.}
...
\todos{Reescribir cierre final: la evidencia mas fuerte debe incluir ANN Sobolev para griegas y CaNN; PINN Sobolev+ACV queda como mejora relevante en el bloque no supervisado/physics-informed.}
