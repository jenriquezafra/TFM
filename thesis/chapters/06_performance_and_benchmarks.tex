...
\begin{table}[H]
    ...
    {\begin{tabular}{|P{1.8cm}|P{1.8cm}|P{2.2cm}|P{2.2cm}|P{2.2cm}|}
        ...
        \hline
        Train & \(8.0\times10^5\) & \(2.96\times10^{-7}\) & \(5.44\times10^{-4}\) & 0.96 \\
        \hline
        Validation & \(1.0\times10^5\) & \(4.65\times10^{-6}\) & \(2.16\times10^{-3}\) & 13.21 \\
        \hline
        Test & \(1.0\times10^5\) & \(9.18\times10^{-6}\) & \(3.03\times10^{-3}\) & 18.33 \\
        \hline
        \multicolumn{5}{l}{Source: Own elaboration}
    \end{tabular}}
\end{table}

The test RMSE remains in the \(10^{-3}\) range in absolute IV units. MAPE is larger than the raw IV error scale suggests because relative errors are amplified in low-IV regions; therefore it is read jointly with the spatial diagnostics below rather than as an isolated scalar.
...
Figure~\ref{fig:region_error_mse_ann} makes the same pattern explicit with two panels: left panel (MSE by moneyness bins) and right panel (MSE by maturity bins). In both validation and test curves, the first bin is clearly the most difficult, while errors drop sharply and remain much lower in interior bins. This is consistent with boundary/extrapolation effects and with the smoother behavior observed around central bins.
...
\begin{table}[H]
    \ttabbox[\FBwidth]
    {\caption{CaNN quote-level residual metrics (selected run).}\label{tab:cann_residual_distribution}}
    {\begin{tabular}{|P{2.3cm}|P{2.3cm}|P{2.3cm}|P{2.3cm}|}
        \hline
        \(\mathbf{n}\) & \textbf{MSE} & \textbf{RMSE} & \textbf{MAPE (\%)} \\
        \hline
        35 & \(4.06\times10^{-8}\) & \(2.01\times10^{-4}\) & 0.038 \\
        \hline
        \multicolumn{4}{l}{Source: Own elaboration}
    \end{tabular}}
\end{table}

Residual concentration is mild. As an auxiliary tail diagnostic, the largest local deviations appear around intermediate-to-high moneyness and medium maturities, but their scale remains below \(5\times10^{-4}\) in absolute IV units.
