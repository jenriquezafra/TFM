...
\begin{table}[H]
    \ttabbox[\FBwidth]
    {\caption{Optimizer-selection results for retained ANN candidates. All rows use the same train/validation/test split policy and Liu-style network size.}\label{tab:ann_optimizer_selection}}
    {\small
    \begin{tabular}{|P{2.65cm}|P{1.75cm}|P{1.95cm}|P{1.55cm}|P{1.55cm}|P{1.4cm}|}
        \hline
        \textbf{Candidate} & \textbf{Activation} & \textbf{Update schedule} & \textbf{Val MSE} & \textbf{Test RMSE} & \textbf{Test MAPE} \\
        \hline
        Selected tanh ANN & \texttt{tanh} & Adam & \(4.00\times10^{-6}\) & \(1.86\times10^{-3}\) & 15.88\% \\
        \hline
        ReLU baseline & ReLU & Adam & \(4.65\times10^{-6}\) & \(3.03\times10^{-3}\) & 18.33\% \\
        \hline
        Adam--L-BFGS refinement & ReLU & Adam \(\rightarrow\) L-BFGS & \(4.55\times10^{-6}\) & \(3.07\times10^{-3}\) & 27.17\% \\
        \hline
...
This section reports the benchmark outcomes of the CaNN workflow. The compact calibration setup and diagnostic reading are introduced in Section~\ref{sec:cann_inverse_problem}; here we focus on final metrics and on the selection of the reference calibration run used in the rest of the chapter.

\subsection{Benchmark Snapshot}

The calibration benchmark in this section follows the inverse-problem construction of Section~\ref{sec:cann_setup}: it uses a fixed ANN surrogate (from Section~\ref{sec:ann_pricer_performance}) and one fixed quote universe of 35 contracts (Table~\ref{tab:cann_quote_universe}). The calibrated vector is \((\rho,\kappa,\gamma,\bar v,v_0)\), optimized with Differential Evolution plus polish under the same bounds and controls defined in Sections~\ref{sec:cann_setup}--\ref{sec:cann_search_stability}.
...
\begin{figure}[H]
    ...
    \caption[Composite PINN pricing benchmark versus COS reference]{Composite PINN pricing benchmark versus COS reference. Top-left: PINN-COS scatter. Top-right: global MSE/RMSE/MAPE and relative-error summary. Bottom-left: mean relative absolute-error map over \((m,\tau)\). Bottom-right: empirical CDF of relative absolute pricing errors.}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.90\textwidth]{figures/pinn/greeks/baseline_vs_sobolev_rel_abs_error_maps_10panel.png}
    \caption{Baseline PINN Greek relative-error maps.}
    \label{fig:no_sobolev_greeks_error_maps}
\end{figure}
