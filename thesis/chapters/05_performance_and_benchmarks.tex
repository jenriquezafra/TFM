...
The remaining error is also spatially structured rather than purely scattered. Several panels keep coherent high-error regions or bands after Sobolev training, which indicates that the derivative surface still contains systematic approximation errors in localized parts of the grid. Therefore, the Sobolev ANN should be read as a more stable differentiable surrogate in aggregate, but not as a correction that removes the kink-related short-maturity difficulty.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.90\textwidth]{figures/nn/ann_iv_baseline_vs_sobolev_rel_error_maps_10panel.png}
    \caption{Relative absolute-error maps for ANN-derived Greeks. The two columns compare the ANN baseline and the ANN Sobolev surrogate on the fixed \((m,\tau)\) benchmark grid used for the Greek errors summarized in Table~\ref{tab:ann_sobolev_greek_rmse}. Rows correspond to Delta, Gamma, Vega, Theta, and Rho; the horizontal axis is maturity \(\tau\), the vertical axis is moneyness \(m\), and the shared colorbar reports relative absolute error on a logarithmic scale.}
...
\begin{table}[H]
    \ttabbox[\FBwidth]
    {\caption{CaNN calibration results using ANN baseline and ANN Sobolev surrogates versus Liu et al. On the replicated \(35\)-quote benchmark, all quote weights are equal to one, so the reported weighted MSE coincides with the mean squared IV residual.}\label{tab:cann_vs_liu}}
    {\small
...
\begin{figure}[H]
    ...
    \caption[Baseline PINN pricing benchmark versus reference]{Baseline PINN pricing benchmark versus reference. Top-left: price residual scatter, with \(V_{\mathrm{ref}}\) on the horizontal axis and \(V_{\mathrm{PINN}}-V_{\mathrm{ref}}\) on the vertical axis. Top-right: global MSE/RMSE/MAPE and relative-error summary. Bottom-left: mean relative absolute-error map over \((\tau,m)\), with the color scale representing the cellwise mean relative absolute error. Bottom-right: histogram of relative absolute pricing errors.}
...
\begin{figure}[H]
    ...
    \caption[Sobolev PINN pricing benchmark versus reference]{Sobolev PINN pricing benchmark versus reference. Top-left: price residual scatter, with \(V_{\mathrm{ref}}\) on the horizontal axis and \(V_{\mathrm{PINN}}-V_{\mathrm{ref}}\) on the vertical axis. Top-right: global MSE/RMSE/MAPE and relative-error summary. Bottom-left: mean relative absolute-error map over \((\tau,m)\), with the color scale representing the cellwise mean relative absolute error. Bottom-right: histogram of relative absolute pricing errors.}
...
\begin{figure}[H]
    ...
    \caption[Baseline--Sobolev PINN Greek relative-error maps]{Baseline--Sobolev PINN Greek relative-error maps. The two columns compare the baseline PINN and the Sobolev PINN on the fixed \((m,\tau)\) benchmark grid used for the Greek errors summarized in Table~\ref{tab:ablation_metrics_by_greek}. Rows correspond to Delta, Gamma, Vega, Theta, and Rho; the horizontal axis is maturity \(\tau\), the vertical axis is moneyness \(m\), and the shared colorbar reports relative absolute error on a logarithmic scale.}
...
\begin{figure}[H]
    ...
    \caption[Kink-region PINN Greek relative-error maps]{Kink-region PINN Greek relative-error maps. The three columns compare the baseline PINN, the Sobolev PINN, and the final Sobolev+ACV correction on the local short-maturity ATM grid \(m\in[0.9704,1.0305]\), \(\tau\in[0.005,0.05]\). Rows correspond to Delta, Gamma, Vega, Theta, and Rho; the horizontal axis is maturity \(\tau\), the vertical axis is moneyness \(m\), and each row uses an independent logarithmic colorbar for the relative absolute error of that Greek.}
