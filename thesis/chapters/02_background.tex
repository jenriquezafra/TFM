...
\begin{equation}
    \mathrm{MAE}_{IV}=\frac{1}{N}\sum_{i=1}^{N}\left|\hat\sigma_i-\sigma_i^{\mathrm{ref}}\right|,
    \qquad
    \mathrm{RMSE}_{IV}=\sqrt{\frac{1}{N}\sum_{i=1}^{N}\left(\hat\sigma_i-\sigma_i^{\mathrm{ref}}\right)^2 }.
\end{equation}
Here, $\mathrm{MAE}_{IV}$ and $\mathrm{RMSE}_{IV}$ are the mean absolute error and root mean squared error in IV units, $\hat\sigma_i$ is the model-implied volatility prediction, $\sigma_i^{\mathrm{ref}}$ is the reference implied volatility, and $N$ is the number of evaluation points in the metric. To capture tail behavior, quantiles of absolute IV error are also reported (e.g., $q_{95}$ and $q_{99}$), together with stratified summaries by moneyness and maturity buckets.
