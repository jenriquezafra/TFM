...
\begin{figure}[H]
    ...
    \begin{tikzpicture}[
        ...
        \foreach \layer/\xpos in {1/2.10,2/4.00,3/5.90,4/7.80} {
            ...
            \node[font=\scriptsize] at (\xpos,-1.75) {200 ReLU units};
        }
    ...
    \end{tikzpicture}
    \caption{Fully connected ANN pricer used in the forward Heston implied-volatility surrogate. The eight fixed inputs are mapped through four hidden layers of width \(200\), and the final linear layer returns the predicted implied volatility.}
...
Several optimizer configurations were tested (Adam, mixed schedules, and \texttt{mix\_half}). For model selection, we prioritize out-of-sample MSE/RMSE and MAPE, since this keeps the training objective and the reporting convention aligned with the benchmark chapter.

Under this criterion, the selected reference ANN is \texttt{Liu\_like\_v01} (Adam), while \texttt{MIX\_half\_v01} is the second-best candidate. The two runs have similar validation MSE, but the selected run gives a lower test MSE and lower test MAPE.

\begin{table}[H]
    \ttabbox[\FBwidth]
    {\caption{Reference and runner-up runs used in optimizer selection}\label{tab:ann_optimizer_selection}}
    {\scriptsize
    \begin{tabular}{|P{2.0cm}|P{2.0cm}|P{1.45cm}|P{1.45cm}|P{1.45cm}|P{1.45cm}|}
        \hline
        \textbf{Run} & \textbf{Optimizer mode} & \textbf{Val MSE} & \textbf{Test MSE} & \textbf{Val MAPE} & \textbf{Test MAPE} \\
        \hline
        Liu v01 & Adam & $4.65\times 10^{-6}$ & $9.18\times 10^{-6}$ & 13.21\% & 18.33\% \\
        \hline
        MIX half v01 & Adam $\rightarrow$ L-BFGS & $4.55\times 10^{-6}$ & $9.41\times 10^{-6}$ & 17.36\% & 27.17\% \\
        \hline
        \multicolumn{6}{l}{Source: Own elaboration from \texttt{outputs/runs/*/metrics/eval\_global.csv} and ANN prediction outputs}
    \end{tabular}}
\end{table}

The hyperparameters of the selected reference run (\texttt{Liu\_like\_v01}) are summarized in Table~\ref{tab:ann_selected_hyperparameters}.

\begin{table}[H]
    \ttabbox[\FBwidth]
    {\caption{Training hyperparameters of the selected run (\texttt{Liu\_like\_v01})}\label{tab:ann_selected_hyperparameters}}
